...
def active_splits_path():
    """Return the split file selected for this run."""
    return Path(os.environ.get("BIAS_AUTORESEARCH_SPLITS", "data/splits.json"))


def results_dir(name):
    """Create a results directory, optionally suffixed for alternate split files."""
    suffix = os.environ.get("BIAS_AUTORESEARCH_RESULTS_SUFFIX")
    if suffix is None:
        split_path = active_splits_path()
        suffix = "" if split_path == Path("data/splits.json") else "_%s" % split_path.stem
    path = Path("results") / ("%s%s" % (name, suffix))
    path.mkdir(parents=True, exist_ok=True)
    return path


@lru_cache(maxsize=None)
def _load_splits_from_path(split_path):
    with open(split_path) as f:
        return json.load(f)


def load_splits(path=None):
    """Load occupation/template splits from data/splits.json or BIAS_AUTORESEARCH_SPLITS."""
    split_path = Path(path) if path is not None else active_splits_path()
    return _load_splits_from_path(str(split_path))

"""
(Gemma-2-2B): Head Re-Discovery on Discovery Set

Full head ablation scan on Gemma-2-2B using the first 50 discovery-set
prompts for speed. 

Architecture notes:
  - Gemma-2-2B: 26 layers × 8 heads (GQA: n_key_value_heads=4)
  - hook_z indexes the *query* heads, so we ablate each of the 8 heads
    per layer exactly.
  - SentencePiece tokenizer — gender tokens may differ from GPT-2 BPE,
    but eval_bias / eval_wikitext_ppl in eval_utils handle this.
"""
...
print("=" * 70)
print("(GEMMA-2-2B): HEAD SCAN ON DISCOVERY SET")
print("=" * 70)

# ═══════════════════════════════════════════════════════════════
# LOAD MODEL  (gated HF pattern — load HF objects, then wrap)
# ═══════════════════════════════════════════════════════════════
MODEL_NAME = None
model = None
...
if MODEL_NAME is None:
    raise RuntimeError(
        ...
        "  3. transformers >= 4.38.2 is installed\n"
...
print("  d_model:  %d" % model.cfg.d_model)
print("  d_head:   %d" % model.cfg.d_head)
print("  Total heads to scan: %d" % total_heads)

from eval_utils import get_gender_ids
m_ids, f_ids = get_gender_ids(model)
m_decoded = [model.tokenizer.decode([i]) for i in m_ids]
f_decoded = [model.tokenizer.decode([i]) for i in f_ids]
...
assert all(d.strip().lower() in ["he","him","his","himself"] for d in m_decoded), \
    f"Male token resolution failed: {m_decoded}"
assert all(d.strip().lower() in ["she","her","hers","herself"] for d in f_decoded), \
    f"Female token resolution failed: {f_decoded}"
...
# ═══════════════════════════════════════════════════════════════
# FULL HEAD SCAN  (bias + quick PPL per head)
# ═══════════════════════════════════════════════════════════════
print("\n" + "=" * 70)
...
for layer in range(n_layers):
    for head in range(n_heads):
        ...
        if scanned % 32 == 0 or scanned == total_heads:
...
print("  Gemma top head:       layer %d/%d  = %.0f%% depth, bias_red=%.1f%%, ppl_Δ=%+.1f%%" % (
